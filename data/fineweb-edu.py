...
if args.version == '10B':
    local_dir = 'fineweb-edu_10B'
    remote_name = 'sample-10BT'
elif args.version == '100B':
    local_dir = 'fineweb-edu_100B'
    remote_name = 'sample-100BT'
elif args.version == '350B':
    local_dir = 'fineweb-edu_350B'
...
# init the tokenizer
enc = tokenizers.Tokenizer.from_file('falcon-7b-instruct_tokenizer.json')
eot = enc.token_to_id('<|endoftext|>') # end of text token
def tokenize(doc):
...
with mp.Pool(nprocs) as pool:
    ...
    for tokens in pool.imap(tokenize, fw, chunksize=16):
        ...
        if token_count + len(tokens) < args.shard_size:
            ...
            filename = os.path.join(DATA_CACHE_DIR, f'fineweb-edu_{split}_{shard_index:06d}.bin')
