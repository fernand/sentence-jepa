...
def evaluate_sts_benchmark(model_components, tokenizer, device, dataset_name='stsb', batch_size=32):
    ...
    for sent1, sent2 in zip(sentences1, sentences2):
        tokens1 = tokenizer(sent1, truncation=False, return_tensors='pt')['input_ids']
        tokens2 = tokenizer(sent2, truncation=False, return_tensors='pt')['input_ids']
        token_lengths.append(tokens1.shape[1])
        token_lengths.append(tokens2.shape[1])
...
def main():
    ...
    parser.add_argument('--tokenizer', type=str, default='tiiuae/falcon-7b', 
                        help='Tokenizer to use (should match training)')
